...
header{{padding:12px 24px;border-bottom:1px solid #21262d;display:flex;align-items:center;gap:16px;flex-wrap:wrap;}}
...
@media(max-width:600px){{
  ...
  .header-search .search-wrap{{flex:1;}}
  .header-search input[type=text]{{width:100%;}}
  .chart-grid{{grid-template-columns:1fr!important;}}
  .chart-stack{{padding:10px 14px;}}
...
/* 차트 패널 */
.chart-stack{{background:#161b22;border-bottom:1px solid #21262d;padding:12px 24px;display:none;flex-direction:column;gap:12px;}}
.chart-stack.visible{{display:flex;}}
...
.chart-card-title{{font-size:0.92rem;color:#e6edf3;font-weight:600;}}
.chart-card-close{{background:none;border:none;color:#8b949e;font-size:1rem;cursor:pointer;padding:0 4px;line-height:1;}}
.chart-card-close:hover{{color:#e6edf3;}}
.chart-grid{{display:grid;grid-template-columns:1fr 1fr;gap:12px;}}
.chart-box{{min-width:0;overflow:hidden;}}
.chart-box h3{{font-size:0.78rem;color:#8b949e;margin-bottom:6px;}}
...
input[type=text]{{padding:6px 12px;border-radius:6px;border:1px solid #30363d;
                  background:#ffffff;color:#000000;font-size:0.88rem;width:220px;}}
...
<header>
  ...
  <div class="header-search">
    <div class="search-wrap">
      <input type="text" id="searchInput" placeholder="종목코드 / 종목명 검색 후 Enter"
...
// ── 종목 차트 (최대 3개 스택) ─────────────────────────────────────────────
const MAX_CHARTS = 10;
const chartCards = []; // {{ticker, chart1, chart2, el}}
...
function showChart(ticker) {{
  ...
  // 이미 표시 중인 종목이면 스크롤만
  const existing = chartCards.find(c => c.ticker === ticker);
  if (existing) {{
  ...
  // 카드 DOM 생성
  const cardId = 'card_' + ticker;
  const c1Id   = 'c1_' + ticker;
  const c2Id   = 'c2_' + ticker;
  const card = document.createElement('div');
  card.className = 'chart-card';
  card.id = cardId;
  card.innerHTML = `
    <div class="chart-card-header">
      <span class="chart-card-title">${{d.name || ticker}} (${{ticker}}) — ${{d.date}}</span>
      <button class="chart-card-close" onclick="removeChart('${{ticker}}')">✕</button>
    </div>
    <div class="chart-grid">
      <div class="chart-box"><h3>시가총액 + 20일순매도합산(억)</h3><canvas id="${{c1Id}}"></canvas></div>
      <div class="chart-box"><h3>시가총액 + 수급오실레이터(%)</h3><canvas id="${{c2Id}}"></canvas></div>
    </div>`;

  const stack = document.getElementById('chartStack');

  // 최대 3개 초과 시 가장 오래된 것 제거
  if (chartCards.length >= MAX_CHARTS) {{
    const oldest = chartCards.shift();
    oldest.chart1.destroy();
    oldest.chart2.destroy();
    oldest.el.remove();
  }}
  ...
  const ch1 = new Chart(document.getElementById(c1Id), {{
  ...
  const ch2 = new Chart(document.getElementById(c2Id), {{
    data: {{ labels, datasets: [
      {{ type:'line', label:'시가총액(억)', data:mktcap, yAxisID:'y1',
...
function removeChart(ticker) {{
  const idx = chartCards.findIndex(c => c.ticker === ticker);
  if (idx === -1) return;
  const entry = chartCards.splice(idx, 1)[0];
  entry.chart1.destroy();
  entry.chart2.destroy();
  entry.el.remove();
  if (chartCards.length === 0)
    document.getElementById('chartStack').classList.remove('visible');
}}
